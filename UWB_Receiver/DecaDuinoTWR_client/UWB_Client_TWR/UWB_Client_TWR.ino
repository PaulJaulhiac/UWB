...
void loop()
{
  ...
  switch (state) {
    ...
    case TWR_ENGINE_STATE_EXTRACT_T2_T3: {
      // Extract timestamps t2 and t3 from received data
      t2 = decaduino.decodeUint40(&rxData[1]);
      t3 = decaduino.decodeUint40(&rxData[6]);

      // Calculate time-of-flight (tof) and distance
      tof = (((t4 - t1) & mask) - ((t3 - t2) & mask)) / 2;
      distance = tof * RANGING_UNIT;

      // Calculate skew-corrected tof and distance
      int32_t tof_sk = (((t4 - t1) & mask) - (1 + 1.0E-6 * decaduino.getLastRxSkew()) * ((t3 - t2) & mask)) / 2;
      float distance_sk = tof_sk * RANGING_UNIT;
      ...
      Serial.print(tof_sk);
      Serial.print(",");
      Serial.println(distance_sk);

      // Reset state to INIT for the next measurement
      state = TWR_ENGINE_STATE_INIT;
      break;
    }
